...
public class PurchaseItemModal extends JDialog {

    private static final String API_URL = "http://localhost:8080/products";
    private PurchaseItemDTO selectedItem;

    public PurchaseItemModal() {
    }

    public void purchaseItemActionModal() {
        // Determina se é criação ou edição
        String dialogTitle = "Produtos";
        ...
        JPanel dataPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 5)); // Alinha à esquerda com espaçamento
        dataPanel.add(new JLabel("Quantidade:"));

        JTextField quantitySaleField = new JTextField(10); // Define a largura preferida do campo
        dataPanel.add(quantitySaleField);

        // Painel para o campo de "Quantidade"
//        JPanel unitPricePanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 5)); // Alinha à esquerda com espaçamento
        dataPanel.add(new JLabel("Preço da unidade:"));

        JTextField unitPriceField = new JTextField(10); // Define a largura preferida do campo
        dataPanel.add(unitPriceField);
        ...
        btnSave.addActionListener(e -> {
            ...
            if (selectedRows == 0) {
                ...
                if (selectedRows > 1) {
                    ...
                    double unitPrice;
                    try {
                        quantity = Integer.parseInt(quantitySaleField.getText());
                        unitPrice = Double.parseDouble(unitPriceField.getText());
                    } catch (NumberFormatException ex) {
                        JOptionPane.showMessageDialog(dialog, "Quantidade ou preço inválido.", "Erro", JOptionPane.ERROR_MESSAGE);
                    ...
                    if (selectedProduct != null) {
                        if (unitPrice > 0 || quantity > 0) {
                            double newStock = selectedProduct.getTotalStock() + quantity;
                            selectedProduct.setTotalStock(newStock);
                            selectedItem = new PurchaseItemDTO(selectedProduct, quantity, unitPrice);
                            // Fechar o diálogo e passar os dados
                            dialog.dispose();
                        } else {
                            JOptionPane.showMessageDialog(dialog, "Quantidade ou preço incorreto, informe novamente.", "Erro", JOptionPane.ERROR_MESSAGE);
                        }
                    } else {
    ...
    public PurchaseItemDTO getSelectedItem() {
        return selectedItem;
    }
}
